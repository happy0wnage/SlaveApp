...
public class FolderPath {
    ...
    public void createFolderPath() {
        Backendless.Files.listing(FunctionalActivity.DEFAULT_PATH_ROOT, "*", true, new AsyncCallback<BackendlessCollection<FileInfo>>() {
            @Override
            public void handleResponse(BackendlessCollection<FileInfo> fileInfo) {
                for (FileInfo info : fileInfo.getData()) {
                    ...
                    if (info.getName().equals(folder)) {
                        setPath(publicURL.substring(publicURL.indexOf(FunctionalActivity.DEFAULT_PATH_ROOT)));
                        break;
                    }
